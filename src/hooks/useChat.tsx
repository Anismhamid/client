...
export const ChatProvider = ({
    ...
    const addMessageForUser = useCallback(
        (userId: string, msg: LocalMessage) => {
            setMessages((prev) => {
    ...
    useEffect(() => {
        ...
        const handleIncomingMessage = (msg: LocalMessage) => {
            if (!msg?.from?._id || !msg?.to?._id) return;

            const otherUserId =
                msg.from._id === authId ? msg.to._id : msg.from._id;

            if (!otherUserId || otherUserId === 'unknown') return;

            addMessageForUser(otherUserId, msg);

            if (otherUserId !== currentChatId) {
                setUnreadForUser(otherUserId, (prev) => prev + 1);
            }
        };
    ...
    }, [addMessageForUser, setUnreadForUser, authId, currentChatId]);

...
const PostCard: FunctionComponent<PostCardProps> = memo(
	({
		...
		const handleLike = () => {
			...
			if (onLikeToggle && post._id) {
				onLikeToggle(post._id, newLikedState);
			}
		};
			...
			>
				...
				<Box sx={{ px: 2, py: 1 }}>
					<Stack direction="row" justifyContent="space-between" alignItems="center">
						<Stack direction="row" alignItems="center" spacing={0.5}>
							<Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
								<Favorite sx={{ fontSize: 16, color: "#e41e3f" }} />
				...
				<CardActions sx={{ p: 0 }}>
					<Stack direction="row" sx={{ width: "100%" }}>
						...
						<Button
							fullWidth
							startIcon={isLiked ? <ThumbUp sx={{ color: "#1877f2" }} /> : <ThumbUpOutlined />}
							onClick={handleLike}
							sx={{

...
const WebSiteAdmins: FunctionComponent = () => {
    ...
    const fetchData = async (): Promise<void> => {
        ...
        try {
            ...
            const postsWithUserData: PostWithUser[] = postsData.map(
                (post: Posts) => ({
                    ...post,
    ...
    const calculateStatistics = (
        ...
        const pendingPosts: number = posts.filter(
            (p: PostWithUser) => p.status === 'pending',
        ).length;
        const soldPosts: number = posts.filter(
            (p: PostWithUser) => p.status === 'sold',
        ).length;
        ...
        posts.forEach((post: PostWithUser) => {
            ...
            if (sellerId && post.userData) {
                ...
                sellersMap.set(sellerId as string, {
                    ...
                    totalViews:
                        seller.totalViews + getViewsAsNumber(post.views),
        ...
        const totalViews: number = posts.reduce(
            (sum: number, post: PostWithUser) =>
                sum + getViewsAsNumber(post.views),
            0,
        );
            ...
            <main>
                ...
                >
                    <Box>
                        ...
                        <Chip
                            label={
                                auth.role === 'Admin' ? 'مدير النظام' : 'مشرف'
                            }
                            size='small'
